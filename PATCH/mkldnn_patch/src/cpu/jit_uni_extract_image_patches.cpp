...
void jit_uni_extract_image_patches_fwd_t<isa>::execute_forward() {
    ...
    auto ker = [&](int _b, int _oh, int _ow) {
        long src_batch_id = _b * IC * IH * IW;
        long dst_batch_id = _b * OC * OH * OW;
        ...
        for ( int _kh = 0; _kh < KH; _kh ++ ) {
            ...
            for ( int _kw = 0; _kw < KW; _kw ++ ) {
                ...
                for ( int _iC = 0; _iC < C; _iC ++ ) {
                    for ( int _ix = 0; _ix < X; _ix ++ ) {
                        src_id = src_batch_id + _iC*X*IH*IW + _ih*IW*X + _iw*X + _ix;
                        dst_id = dst_batch_id + _oC*X*OH*OW + _oh*OW*X + _ow*X + _ox;
